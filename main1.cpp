...
{
	std::ofstream rawData ("./imageL.txt", rawData.out | rawData.trunc);
	std::ifstream read ("./imageL.txt", read.in);
	MakeTextFile_Left(rawData, IMAGENUM);
	std::ofstream rawData ("./imageR.txt", rawData.out | rawData.trunc);
	std::ifstream read ("./imageR.txt", read.in);
	MakeTextFile_Right(rawData, IMAGENUM);
	...
	while(true)
	{
		while(realFrame < ESSENTIALFRAME)
		{
			...
			else if(realFrame == ESSENTIALFRAME-1)	// 2-viewSFM(2)
			{
				...
				for(int i = 0; i<gP; i++)
				{
					...
					m2 = svo::MultiplyMat(m2,globalRTMat[i]);
		...
		if(mapPointsA.mworldMapPointsV.size() == localTrackPointsA.at(lTPA).mfeatures.size())
		{
			...
			angularVelocity = svo::RotationAngle(globalRMat.at(gP-2), globalRMat.at(gP-1));

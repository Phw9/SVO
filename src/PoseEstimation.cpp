...
svo::StrctureFromMotion::StrctureFromMotion() 
: mEssential{cv::Mat()}, mRotation{cv::Mat()}, mTranslation{cv::Mat()}, mCombineRt{cv::Mat()}{}

bool svo::StrctureFromMotion::CreateEssentialMatrix(const std::vector<cv::Point2f>& pts1, const std::vector<cv::Point2f>& pts2, const cv::InputArray& K)
{
...
bool svo::StrctureFromMotion::GetEssentialRt(const cv::InputArray& E, const cv::InputArray& K, const std::vector<cv::Point2f>& pts1, const std::vector<cv::Point2f>& pts2)
{
...
bool svo::StrctureFromMotion::CombineRt()
{
    cv::Mat temp = cv::Mat();
    temp = mRotation.t();
...
void svo::StrctureFromMotion::GetRTvec()
{
...
svo::PoseEstimation::PoseEstimation(): 
            mRotation{cv::Mat()}, mTranslation{cv::Mat()}, mCombineRt{cv::Mat()}{};

void svo::PoseEstimation::solvePnP(const std::vector<cv::Point3f>& objectPoints,
...
void svo::PoseEstimation::GetRMatTPose()
{
...
bool svo::PoseEstimation::CombineRt()
{
    cv::Mat temp = cv::Mat();
    temp = mRotation.t();
    temp.push_back(mTranslation.t());
...
cv::Mat svo::MultiplyMat(const cv::Mat& R1, const cv::Mat& R2)
{
        double data[] = {0,0,0,1};
...
double svo::RotationAngle(const cv::Mat& R1, const cv::Mat& R2)
{
